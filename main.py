...
def init_db():
    ...
    # Profit Growth screener results: stocks with rising quarterly profit
    # (as many recent quarters as yfinance provides, usually 3-4) AND
    # trading 15%+ below their 52-week high.
...
def fetch_nifty500_universe(conn, progress_callback=None):
    """
    Returns the Nifty 500 constituent list specifically (NOT the full
    ~2000 stock NSE universe). This is what Top Gainers / Top Losers
    are computed from, per your instruction that movers should be
    Nifty 500 only. Order of attempts: live NSE fetch -> cached
    nifty500_cache table -> small fixed fallback list.
    """
    if progress_callback:
        progress_callback("Fetching Nifty 500 list...")
    live = _fetch_nifty500_only(conn)
    if live:
        return live
    cursor = conn.cursor()
...
def run_profit_growth_scan(conn, progress_callback=None):
    """
    Screener: scans the Nifty 500 universe for stocks that have posted
    rising quarterly net profit for every recent quarter yfinance has
    data for (usually the last 3-4 reported quarters), AND are
    currently trading at least 15% below their 52-week high (i.e.
    fundamentally improving but still "on sale" price-wise).

    This is genuinely slow (2 API calls x 500 stocks) so it is not run
    on every "Update Market Data" tap - only manually from Settings, or
    once a day automatically during the 4 PM background sync.
    """
    universe = fetch_nifty500_universe(conn, progress_callback)
    total = len(universe)
    found = []
    ...
    for i, (symbol, company_name) in enumerate(universe, 1):
        try:
            ...
            if q is None or q.empty or "Net Income" not in q.index:
                continue
            profit_row = q.loc["Net Income"].dropna()
            # yfinance returns columns newest-first; reverse to oldest -> newest
            profits_oldest_first = list(profit_row.iloc[::-1])
            if len(profits_oldest_first) < 3:
                continue
            is_rising = all(
                profits_oldest_first[j] < profits_oldest_first[j + 1]
                for j in range(len(profits_oldest_first) - 1)
            )
            if not is_rising:
                continue
            ...
            if week52_high <= 0:
                continue
            pct_below_high = round((week52_high - price) / week52_high * 100, 2)
            ...
            if pct_below_high >= 15:
                found.append({
                    ...
                    "quarters_of_growth": len(profits_oldest_first),
                })
        except Exception:
            continue

        if progress_callback and i % 20 == 0:
            progress_callback(f"Profit Growth scan: {i}/{total} checked, {len(found)} found...")

    cursor = conn.cursor()
    cursor.execute("DELETE FROM profit_growth_stocks")
    for m in found:
        cursor.execute(
            "INSERT OR REPLACE INTO profit_growth_stocks "
            "(symbol, company_name, price, week52_high, pct_below_high, quarters_of_growth, scan_date) "
            "VALUES (?,?,?,?,?,?,?)",
            (m["symbol"], m["company_name"], m["price"], m["week52_high"],
             m["pct_below_high"], m["quarters_of_growth"], scan_date),
        )
    conn.commit()
    set_setting(conn, "profit_growth_last_scan", datetime.now().strftime("%d %b %Y, %H:%M"))
    if progress_callback:
        progress_callback(f"Profit Growth scan complete: {len(found)} stocks found.")
    return found


def get_profit_growth_stocks(conn, limit=None):
    cursor = conn.cursor()
    q = ("SELECT symbol, company_name, price, week52_high, pct_below_high, quarters_of_growth, scan_date "
...
def fetch_top_movers_from_nse(name_lookup):
    ...
    try:
        ...
        def extract(payload):
            # "allSec" = All Securities on NSE (broadest feed NSE publishes
            # here). We filter down to only symbols present in name_lookup
            # so callers control exactly which universe (e.g. Nifty 500)
            # gainers/losers are computed from.
            rows = payload.get("allSec", {}).get("data", [])
            out = []
            for r in rows:
                try:
                    sym = r["symbol"]
                    if sym not in name_lookup:
                        continue
                    out.append({
                        "symbol": sym,
...
def perform_full_market_sync(conn, progress_callback=None):
    ...
    try:
        ...
        # Full ~2000 stock universe: powers search / watchlist-add only.
        fetch_full_market_universe(conn, progress_callback)

        # Nifty 500 universe: Top Gainers/Losers are computed from THIS
        # list only, per your instruction (not the full ~2000 universe).
        universe = fetch_nifty500_universe(conn, progress_callback)
        ...
        # Update watchlist with latest price / change - ALWAYS refetch every
        # watchlist symbol directly (not just ones missing from the movers
        # lists), so every stock you add shows a live price + % change.
        ...
        if fav_symbols:
            if progress_callback:
                progress_callback(f"Updating your {len(fav_symbols)} watchlist stock(s)...")
            watchlist_prices = fetch_watchlist_prices(conn, fav_symbols, progress_callback)
            for symbol, m in watchlist_prices.items():
                movers_by_symbol[symbol] = m
...
def fetch_watchlist_prices(conn, symbols, progress_callback=None):
    """
    Dedicated, robust price fetch for watchlist stocks - runs on every
    sync for EVERY watchlist symbol (not just ones missing from the
    Nifty 500 gainers/losers lists). Uses one yf.Ticker call per symbol
    instead of a bulk yf.download(), which is slower for many symbols
    but far more reliable (bulk multi-ticker downloads have repeatedly
    returned inconsistent column shapes that silently dropped stocks
    like newly-listed ones, e.g. TATACAPITAL).
    Returns {symbol: {"price": float, "pct_change": float}}.
    """
    results = {}
    for s in symbols:
        try:
            hist = yf.Ticker(f"{s}.NS").history(period="5d")
            ...
            results[s] = {"price": round(last_close, 2), "pct_change": pct}
        except Exception:
            continue
        if progress_callback:
            progress_callback(f"Updating watchlist: {s}...")
    return results
...
def main(page: ft.Page):
    ...
    # ---------- STOCK DETAILS PAGE ----------
    def build_chart_url(symbol, source):
        if source == "google":
            return f"https://www.google.com/finance/quote/{symbol}:NSE"
        # TradingView's public embeddable chart widget - no API key needed.
        return (
            f"https://s.tradingview.com/widgetembed/?symbol=NSE%3A{symbol}"
            f"&interval=D&theme=dark&style=1&hidesidetoolbar=1&saveimage=0"
            f"&toolbarbg={SURFACE.replace('#', '')}&studies=[]&hideideas=1"
        )
    ...
    def show_stock_details(symbol, company_name, sector, price):
        ...
        chart_state = {"source": "tradingview"}
        ...
        def on_copy_click(e):
            copy_to_clipboard(f"{symbol} - {company_name} - Rs.{price_text.value}", "Stock info")

        price_text = ft.Text(f"Rs.{price:,.2f}" if price else "Fetching live price...",
                              size=34, weight=ft.FontWeight.W_900, color=TEXT_PRIMARY)
        change_badge = ft.Container(visible=False)
        ...
        def load_live_price():
            try:
                ...
                if len(closes) >= 2:
                    ...
                    price_text.value = f"Rs.{last_close:,.2f}"
                    change_badge.content = ft.Row(
                        [
                            ft.Icon(Icons.ARROW_UPWARD if up else Icons.ARROW_DOWNWARD, size=14, color=color),
                            ft.Text(f"{pct:+.2f}%", size=14, color=color, weight=ft.FontWeight.W_700),
                        ],
                        spacing=3, tight=True,
                    )
                    change_badge.bgcolor = f"{color}22"
                    change_badge.padding = ft.padding.symmetric(horizontal=8, vertical=3)
                    change_badge.border_radius = 8
                    change_badge.visible = True
                    page.update()
            except Exception:
                pass

        threading.Thread(target=load_live_price, daemon=True).start()

        chart_view = ft.WebView(
            url=build_chart_url(symbol, chart_state["source"]),
            expand=True,
        )

        def switch_chart_source(source):
            def _switch(e):
                chart_state["source"] = source
                chart_view.url = build_chart_url(symbol, source)
                tv_btn.bgcolor = ACCENT if source == "tradingview" else SURFACE_ALT
                tv_btn.color = Colors.WHITE if source == "tradingview" else TEXT_SECONDARY
                gf_btn.bgcolor = ACCENT if source == "google" else SURFACE_ALT
                gf_btn.color = Colors.WHITE if source == "google" else TEXT_SECONDARY
                page.update()
            return _switch

        tv_btn = ft.ElevatedButton(
            "TradingView", bgcolor=ACCENT, color=Colors.WHITE, expand=True,
            on_click=switch_chart_source("tradingview"),
            style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=10), padding=10, elevation=0,
                                  text_style=ft.TextStyle(size=12, weight=ft.FontWeight.W_600)),
        )
        gf_btn = ft.ElevatedButton(
            "Google Finance", bgcolor=SURFACE_ALT, color=TEXT_SECONDARY, expand=True,
            on_click=switch_chart_source("google"),
            style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=10), padding=10, elevation=0,
                                  text_style=ft.TextStyle(size=12, weight=ft.FontWeight.W_600)),
        )
            ...
            content=ft.Column([
                ...
                ft.Container(
                    ...
                    content=ft.Column([
                        ...
                        ft.Container(height=10),
                        ft.Row([price_text, change_badge], spacing=10, vertical_alignment=ft.CrossAxisAlignment.CENTER),
                        ft.Text(company_name, color=TEXT_SECONDARY, size=14),
                    ]),
                ),
                ft.Container(height=14),
                ft.Row([tv_btn, gf_btn], spacing=8),
                ft.Container(height=10),
                ft.Container(
                    bgcolor=SURFACE, border_radius=14, border=ft.border.all(1, BORDER),
                    height=380, clip_behavior=ft.ClipBehavior.HARD_EDGE,
                    content=chart_view,
                ),
                ft.Container(height=14),
                ft.ElevatedButton(
    ...
    def on_update_market_data(e):
        ...
        def do_sync():
            ...
            if success:
                ...
                # Profit Growth scan is slow (500 stocks), so only run it
                # here if it hasn't run in the last ~20 hours - keeps
                # repeated taps of this button fast.
                last_scan = get_setting(db_conn, "profit_growth_last_scan")
                should_scan = True
                if last_scan:
                    try:
                        last_dt = datetime.strptime(last_scan, "%d %b %Y, %H:%M")
                        should_scan = (datetime.now() - last_dt).total_seconds() > 20 * 3600
                    except Exception:
                        should_scan = True
                if should_scan:
                    run_profit_growth_scan(db_conn, progress_callback)
                    refresh_home_profit_growth()
                    page.update()
    ...
    home_movers_placeholder = ft.Container()
    home_profit_growth_placeholder = ft.Container()
    ...
    home_screen = ft.Container(
        ...
        content=ft.Column(
            [
                ...
                home_movers_placeholder,
                ft.Container(height=14),
                home_profit_growth_placeholder,
                ft.Container(height=26),
    ...
    def refresh_home_movers():
        refresh_date_dropdown()
        render_mover_list()

    movers_section = ft.Container(
    ...
    # ---------- PROFIT GROWTH SECTION (embedded on Home) ----------
    def build_profit_growth_row(rank, symbol, company_name, price, week52_high, pct_below_high, quarters):
        return ft.Container(
            padding=ft.padding.symmetric(horizontal=12, vertical=10),
            margin=ft.margin.only(bottom=6),
            bgcolor=SURFACE,
            ...
            content=ft.Row(
                [
                    ft.Container(
                        content=ft.Text(str(rank), size=12, weight=ft.FontWeight.BOLD, color=TEXT_PRIMARY),
                        bgcolor=SURFACE_ALT, width=26, height=26, border_radius=13,
                        alignment=ft.alignment.center, border=ft.border.all(1, BORDER),
                    ),
                    ft.Column(
                        [
                            ft.Text(symbol, weight=ft.FontWeight.BOLD, size=14, color=TEXT_PRIMARY),
                            ft.Text(f"{quarters} quarters of rising profit", size=11, color=TEXT_SECONDARY),
                        ],
                        spacing=0, expand=True,
                    ),
                    ft.Column(
                        [
                            ...
                            ft.Container(
                                padding=ft.padding.symmetric(horizontal=6, vertical=1),
                                border_radius=6, bgcolor=f"{GOLD}22",
                                content=ft.Text(f"{pct_below_high:.1f}% below high", size=11, color=GOLD, weight=ft.FontWeight.W_700),
                            ),
                        ],
                        horizontal_alignment=ft.CrossAxisAlignment.END, spacing=3,
                    ),
    ...
    profit_growth_body = ft.Column(spacing=0)
    profit_growth_subtitle = ft.Text("Not scanned yet", size=12, color=TEXT_MUTED)

    def refresh_home_profit_growth():
        profit_growth_body.controls.clear()
        rows = get_profit_growth_stocks(db_conn, limit=10)
        last_scan = get_setting(db_conn, "profit_growth_last_scan")
        profit_growth_subtitle.value = (
            f"Rising profit, 15%+ below 52-week high - last scanned {last_scan}"
            if last_scan else
            "Rising profit, 15%+ below 52-week high - runs automatically once a day"
        )
        if not rows:
            profit_growth_body.controls.append(
                ft.Text("No matching stocks found yet.", color=TEXT_MUTED, size=12)
            )
        else:
            for i, row in enumerate(rows, 1):
                symbol, company_name, price, week52_high, pct_below_high, quarters, scan_date = row
                profit_growth_body.controls.append(
                    build_profit_growth_row(i, symbol, company_name, price, week52_high, pct_below_high, quarters)
                )

    profit_growth_section = ft.Container(
        bgcolor=SURFACE, border_radius=16, border=ft.border.all(1, BORDER),
        padding=16,
        content=ft.Column(
            [
                ft.Row([ft.Icon(Icons.TRENDING_UP, color=GOLD, size=18),
                        ft.Text("PROFIT GROWTH", size=14, weight=ft.FontWeight.W_800, color=TEXT_PRIMARY)]),
                profit_growth_subtitle,
                ft.Container(height=10),
                profit_growth_body,
            ]
        ),
    ...
    # Wire the two Home placeholders (declared before home_screen was built)
    # to the actual sections now that they exist.
    home_movers_placeholder.content = movers_section
    home_profit_growth_placeholder.content = profit_growth_section
    ...
    profit_growth_settings_status = ft.Text(
        f"Last scanned: {get_setting(db_conn, 'profit_growth_last_scan', 'never')}",
        size=12, color=TEXT_MUTED,
    )

    def on_run_profit_growth_scan(e):
        profit_growth_scan_btn.disabled = True
        profit_growth_scan_btn.text = "Scanning..."
        page.update()

        def progress_callback(msg):
            profit_growth_settings_status.value = msg
            page.update()

        def do_scan():
            run_profit_growth_scan(db_conn, progress_callback)
            refresh_home_profit_growth()
            profit_growth_scan_btn.disabled = False
            profit_growth_scan_btn.text = "Run Profit Growth Scan Now (Slow)"
            page.update()

        threading.Thread(target=do_scan, daemon=True).start()

    profit_growth_scan_btn = _premium_button(
        "Run Profit Growth Scan Now (Slow)", Icons.TRENDING_UP, on_run_profit_growth_scan, primary=False
    )
    ...
    settings_screen = ft.Container(
        ...
        content=ft.Column(
            [
                ...
                _section_card(
                    "Profit Growth Scanner", "Runs automatically once a day - or trigger it manually here",
                    [
                        ft.Text(
                            "Finds Nifty 500 stocks with rising quarterly profit that are still "
                            "trading 15%+ below their 52-week high. Checks ~500 stocks one by one "
                            "so it can take several minutes.",
                            size=12, color=TEXT_MUTED,
                        ),
                        profit_growth_scan_btn, profit_growth_settings_status,
                    ],
                ),
    ...
    def background_auto_sync():
        try:
            ...
            if now.hour >= 16 and last_sync != today_str:
                ...
                if success:
                    send_telegram_movers_update(db_conn, progress_callback)
                    # Profit Growth scan is slow (500 stocks) - only run it
                    # once, as part of this once-a-day background sync.
                    run_profit_growth_scan(db_conn, progress_callback)
                    refresh_home_profit_growth()
                    page.update()
        except Exception:
