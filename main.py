...
# Optional libraries – fallback if not installed
try:
...
# Fallback universe (only if no internet and no cache)
FALLBACK_UNIVERSE = [
...
# ---------- MOVERS SYNC (with Nifty 500 filter) ----------
...
def _scan_full_universe_for_movers(conn, universe, progress_callback=None):
    ...
    if progress_callback:
        progress_callback(f"Downloading price data for {len(tickers)} stocks (may take a few minutes)...")
    movers = []
    data_date = None
    batch_size = 250
...
def main(page: ft.Page):
    # Global error handler – if anything crashes, show error screen
    try:
        ...
        def show_stock_details(symbol, company_name, sector, price):
            ...
            if WEBVIEW_AVAILABLE:
                try:
                    ...
                    webview = WebView(url=chart_url, expand=True)
        ...
        # Market cap ranking (stub – you can expand)
        ...
        def on_fetch_market_caps(e):
            ...
            page.update()
            def do_fetch():
                # Placeholder – you can implement the full scan if needed
                time.sleep(2)
                cap_btn.disabled = False
                cap_btn.text = "Update Market-Cap Ranking (Slow)"
                set_setting(db_conn, "market_cap_last_updated", datetime.now().strftime("%d %b %Y, %H:%M"))
                market_cap_status_text.value = "Updated now."
                page.update()
        ...
        # Global error handler – show error screen with traceback
        ...
        page.add(ft.Container(
            ...
            content=ft.Column([
                ...
                ft.Text(str(e), size=14, color=TEXT_SECONDARY, selectable=True),
                ft.Text(error_msg, size=10, color=TEXT_MUTED, selectable=True),
            ], horizontal_alignment=ft.CrossAxisAlignment.CENTER)
        ))
        page.update()
